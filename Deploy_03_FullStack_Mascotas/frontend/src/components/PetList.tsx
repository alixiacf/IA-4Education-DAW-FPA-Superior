...
export default function PetList({ userId, onDelete }: PetListProps) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      alert('Error al eliminar mascota: ' + (error as any).message);
    }
  };
      ...
      {pets.map((pet) => (
        <div key={pet.id} className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="p-4">
            ...
            {/* Para depuración, vamos a mostrar el ID */}
            <p className="text-xs text-gray-500 mt-2">ID: {pet.id || pet._id || 'Sin ID'}</p>
            ...
            <button
              onClick={() => {
                ...
                if (petId) {
                  ...
                  alert('Esta mascota no tiene un ID válido');
                }
              }}
